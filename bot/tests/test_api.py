...
def test_spa_fallback_blocks_path_traversal(
    ...
    # The SPA fallback builds a path from user input; it must not let a
    # traversal sequence escape the dist dir and read arbitrary files.
    dist = tmp_path / "dist"
    dist.mkdir()
    ...
    app = create_api_app(settings=settings, session_factory=session_factory, notifier=None)
    # raise_server_exceptions=False so a 500 surfaces as a response, not an error.
    spa_client = TestClient(app, raise_server_exceptions=False)

    # URL-encoded "../secret.txt" so the client does not normalize it away.
